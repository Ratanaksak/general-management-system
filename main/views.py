...
def add_person(request):
    ...
    if request.GET.get('id'):
        person = get_object_or_404(Person, id=request.GET['id'], user=request.user)

    if request.method == 'POST':
        form = PersonForm(request.POST, instance=person)
        if form.is_valid():
            new_person = form.save(commit=False)
            new_person.user = request.user  # 👈 Assign the logged-in user
            new_person.save()
            return redirect('persons_list')  # 👈 Redirect after save (no server error 500)
    else:
        form = PersonForm(instance=person)

    persons = Person.objects.filter(user=request.user)
    totals = persons.aggregate(
...
def render_persons(request):
    persons = Person.objects.filter(user=request.user)  # 👈 only own data
...
def search_person(request):
    ...
    if search:
        persons = Person.objects.filter(name__icontains=search, user=request.user)
    else:
        persons = Person.objects.filter(user=request.user)
...
def edit_person(request, id):
    person = get_object_or_404(Person, id=id, user=request.user)  # 👈 check user match
    if request.method == 'POST':
        form = PersonForm(request.POST, instance=person)
        if form.is_valid():
            form.save()
...
def delete_person(request, id):
    # Get the person object or return a 404 error if not found
    person = get_object_or_404(Person, id=id, user=request.user)

    # Delete the person object
    person.delete()

    # Redirect to the list of persons after deletion
    return redirect('add_person')  # Replace 'persons_list' with your URL name for the list of persons
